...
$$(document).on('page:init', '.page[data-name="calendario"]', function (e) {
  // Do something here when page with data-name="about" attribute loaded and initialized
  $$("#demo-calendar-modal").on("click", abriCalendario);
  ...
  function abriCalendario() {
    ...
    var db = firebase.firestore();
    var colFechas = db.collection('nuevasFechas');
    datos.nuevaFecha = $$('#darFecha').val();  
    datos.fecha = $$('#demo-calendar-modal').val();
    
    colFechas.doc(email).set(datos);
    db.collection(nuevasFechas).doc(email).set(datos);
  }
...
$$("#traerSemana").on("click", function(){

  colmSemana = firebase.firestore();
  var docRef = colmSemana.doc(email).collection("miSemana");
  docRef.get().then(function(doc) 
  {    
    doc.forEach(element => 
    {
